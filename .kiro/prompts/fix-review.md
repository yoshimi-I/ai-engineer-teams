---
name: fix-review
description: PR の REQUEST_CHANGES や CI 失敗を読み取り、修正して再 push するループ。
---
# レビュー指摘の自動修正ループ
...
#### コンフリクト（🔴 マージ失敗）の場合
1. `git fetch origin` → `BASE_BRANCH=$(gh pr view <number> --json baseRefName --jq '.baseRefName')` → `GIT_EDITOR=true git rebase "origin/${BASE_BRANCH}"`
...
```
- `APPROVED` → base branch が `${KIRO_INTEGRATION_BRANCH:-develop}` の場合のみ即座にマージ:
  ```bash
