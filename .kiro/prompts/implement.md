...
UIページやAPIエンドポイントを実装した場合、PR作成前に以下を確認する：

1. **ルーティング登録**: 新しいページを作った場合、App.tsx（またはルーター設定）にルートが追加されているか
2. **API URL一致**: フロントエンドのfetch先URLとバックエンドのルート定義が一致しているか
3. **認証トークン伝播**: ログイン後にトークンを保存し、APIクライアントがそれを送信しているか
4. **CORS/Proxy**: フロントエンドとバックエンドが別ポートの場合、proxy設定またはCORS middlewareがあるか
5. **DB接続**: in-memory Map/配列ではなく、実際のDB（またはORM）に接続しているか（scaffoldのstubは早期に差し替える）

これらが欠けている場合、**自分のPRで修正するか、別issueとして報告する**。
「自分のissueのスコープ外」を理由に無視してはならない。統合が壊れたまま放置すると後で全体が動かなくなる。
