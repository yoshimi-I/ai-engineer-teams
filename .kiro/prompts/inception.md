...
1. `references/design-artifacts.md` を読む
2. 以下の成果物を作成し、**各成果物ごとにユーザーの承認を得る**:

### DB設計（DBを使う場合）
- mermaid ER図を `aidlc-docs/inception/design/er-diagram.md` に作成
- テーブル定義（カラム名、型、制約、インデックス）を提示
- **ユーザーが承認するまで修正を繰り返す**

### API設計（バックエンドがある場合）
- エンドポイント一覧（メソッド、パス、リクエスト/レスポンス型）を提示
- 認証フロー（どのエンドポイントが認証必須か）を明示
- **ユーザーが承認するまで修正を繰り返す**
- 承認後に `aidlc-docs/inception/design/api-spec.md` に保存

### UI設計（フロントエンドがある場合）
- 主要画面のワイヤーフレームをHTML/CSSで作成し、ファイルパスを提示
- 画面遷移図（mermaid）を提示
- コンポーネント構成（どのページにどのコンポーネントがあるか）を提示
- **ユーザーが承認するまで修正を繰り返す**
- 承認後に `aidlc-docs/inception/design/` に保存
