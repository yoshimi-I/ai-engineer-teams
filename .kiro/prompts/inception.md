...
## ステージ5: ドメインモデリング（条件付き — バックエンドがある場合）

1. `references/domain-modeling.md` を読む
2. 要件からイベントストーミングを**AI側で実施**:
   - ドメインイベント洗い出し（過去形で列挙）
   - コマンド特定（何がイベントを引き起こすか）
   - 集約の特定（整合性の境界）
   - 境界づけられたコンテキストの決定
3. HTML成果物（`event-storming.html`）を生成してユーザーに提示
4. **ユーザーが承認するまで議論・改善を繰り返す**
5. 承認後に `aidlc-docs/inception/design/event-storming.html` をコミット
6. ドメインモデルの結果を次のアーキテクチャ設計に引き継ぐ
   （コンテキスト → モジュール構成、集約 → DB設計の境界）

## ステージ6: アーキテクチャ設計（条件付き）
...
3. 実行する場合: チャットでASCII図を使ってアーキテクチャを提案
   ...
   コンポーネントライブラリはそれに従う）
   （ステージ5のドメインモデルがあれば、モジュール構成・DB境界はそれに従う）
4. **ユーザーが承認するまで議論・改善を繰り返す**
5. 承認後にドキュメントを生成

## ステージ7: 設計成果物レビュー（条件付き — DB/API/UIがある場合）
