...
### 5. 設計承認済みの範囲だけ issue を生成する

**issue 生成の並行制限ルール:**

| カテゴリ | 設計承認前に生成OK | 理由 |
|---------|-------------------|------|
| Phase 0: scaffold / CI / infra | ✅ | 設計に依存しない |
| Phase 0: Dockerfile / CD | ✅ | 設計に依存しない |
| Phase 1: Walking Skeleton | ⚠️ DB+API設計の承認後のみ | スキーマ変更で全部作り直しになる |
| Phase 2: 機能実装 | ❌ DB+API+UI全承認後のみ | 設計変更の手戻りが最大 |
| Phase 3-4: Hardening / Release | ❌ Phase 2 の issue 生成後 | 機能が確定してから |

**実行順序:**

```
1. Phase 0 の issue を即座に生成（設計不要）
2. DB設計レビュー → ユーザー承認待ち
3. API設計レビュー → ユーザー承認待ち
4. UI設計レビュー → ユーザー承認待ち
5. 承認された設計に基づいて Phase 1-2 の issue を生成
```

Phase 0 の issue はステージ6の設計レビューと**並行して**パイプラインが
実装を開始できる。設計レビュー中もインフラ構築は進む。

**禁止**: 設計が未承認の状態で機能実装の issue を生成すること。
設計変更が入ると issue の受け入れ基準・変更対象・依存関係が全て崩れる。
