...
<td align="center">👀<br><b>Watch-Main</b><br><sub>develop監視 → E2E → main昇格</sub></td>
...
Agent 1,2: /implement ──→ PR作成（pre-commitでlint/test通過済み）
                 ...
                 developにマージ
                      │
                      ▼
                 Agent 7: /watch-main（develop監視→E2E→main昇格）
                      │
                 バグ発見? → issue作成 → Agent 1,2 が拾う
                                        ▲
                                        │
                   Agent 8: /e2e-bug-hunt（Playwright巡回）
                   Agent 9: /ui-audit（デザイン品質監査）

Agent 10: /improve（改善issue自動生成、10分間隔）
Agent 11: /dev-server（開発サーバー常駐）
```
